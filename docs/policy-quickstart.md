...
The Approval Gate is a desktop card with **Deny**, **Allow for session**, and **Approve Once**. **Allow for session** lasts until the launcher exits or 60 idle minutes pass. If no desktop is available, the request **fails closed**.
