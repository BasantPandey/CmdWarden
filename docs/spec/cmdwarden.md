...
- **Transient reuse ([#131](https://github.com/BasantPandey/CmdWarden/issues/131)):** a human `AllowOnce` / `Deny` is reused, in memory only, for an exact retry (same client pid + start time, tool, class, secret name, command line) inside 5 minutes (`CW_TRANSIENT_REUSE_SECONDS`); audited with reason `TransientReuse`. Policy auto-allow and `Unavailable` are recomputed every call.
