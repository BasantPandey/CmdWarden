...
3. Tighten with `cw policy set` only when defaults are not enough ([UC3](use-cases/agent-reads-github-writes-need-approval.md)).
4. Periodically open CmdWarden Vault (**Detectors**, **Secret Usage**) or run `cw scan` / `cw audit` ([UC7](use-cases/audit-and-scan.md)).
